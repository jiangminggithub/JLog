...
package com.jm.jlog.printer.file;
 ...
 * The policy of clearing will be monitored in real time.
 * <p>
 * Use the {@link Builder} to construct a {@link ConditionMonitorFilePrinter} object.
 *
 * @since 1.0.0
 */
public class ConditionMonitorFilePrinter implements Printer {
    ...
    public ConditionMonitorFilePrinter(Builder builder) {
    ...
    private void doPrintln(long timeMillis, int logLevel, String tag, String msg) {
        ...
        // RollBack LogFile
        boolean rollBackNecessary = rollBackLogFileIfNecessary(lastFile);
        if (rollBackNecessary) {
            if (!writer.open(lastFileName)) {
                throw new IllegalStateException("File open fail, please check the file = " + lastFileName);
            }
        }
    ...
    /**
     * Rollback log file if should clean follow strategy.
     */
    private boolean rollBackLogFileIfNecessary(File file) {
        if (cleanStrategy.shouldClean(file)) {
            file.delete();
            return true;
        }
    ...
    /**
     * Builder for {@link ConditionMonitorFilePrinter}.
     */
    public static class Builder {
         ...
         * @return the builder
         * @deprecated {@link Flattener} is deprecated, use {@link #flattener(Flattener)} instead,
         * since 1.6.0
         */
        @Deprecated
        public Builder logFlattener(final Flattener flattener) {
            return flattener(new Flattener() {
                @Override
                public CharSequence flatten(long timeMillis, int logLevel, String tag, String message) {
                    return flattener.flatten(timeMillis, logLevel, tag, message);
                }
            });
        }
        ...
        /**
         * Build configured {@link ConditionMonitorFilePrinter} object.
         *
         * @return the built configured {@link ConditionMonitorFilePrinter} object
         */
        public ConditionMonitorFilePrinter build() {
            fillEmptyFields();
            return new ConditionMonitorFilePrinter(this);
        }
