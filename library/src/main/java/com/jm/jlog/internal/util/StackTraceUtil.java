...
package com.jm.jlog.internal.util;
...
public class StackTraceUtil {
  ...
  private static final String JLog_STACK_TRACE_ORIGIN;

  static {
    // Let's start from JLog library.
    String JLogClassName = JLog.class.getName();
    JLog_STACK_TRACE_ORIGIN = JLogClassName.substring(0, JLogClassName.lastIndexOf('.') + 1);
  }
  ...
  /**
   * Get the real stack trace, all elements that come from JLog library would be dropped.
   *
   * @param stackTrace the full stack trace
  ...
  private static StackTraceElement[] getRealStackTrack(StackTraceElement[] stackTrace,
    ...
    for (int i = allDepth - 1; i >= 0; i--) {
      ...
      if (className.startsWith(JLog_STACK_TRACE_ORIGIN)
